...
class AppContainer(context: Context) {
    ...
    /** One read of `/access/check` (decision 60, revised): false if refused, null if no answer. */
    private suspend fun accessAllowed(): Boolean? = connection.hold {
        withTimeoutOrNull(ACCESS_CHECK_TIMEOUT_MS) {
            try {
                remote.read(RemoteWrites.ACCESS_CHECK)
                true
    ...
    private companion object {
        ...
        /** Past this the access check has no verdict, and the account carries on as it was. */
        const val ACCESS_CHECK_TIMEOUT_MS = 15_000L
    }
}
