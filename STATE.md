...
**`v0.9.0` released 2026-09-24, and `deploy.yml` was green on its first run** (run 35985594685,
both jobs). Tagged deliberately *before* 1.0, on the owner's choice, so that the workflow's
first execution could not cost the 1.0 tag — a published tag is not moved (and here nothing
even enforces that: there are no rulesets, open question 3). It also gives 1.0 a previous
release to install over, which is the one acceptance criterion that could not otherwise be
tested at all. `main` did not exist until this release: the repository had only ever had `dev`,
whose default branch it still is, so `main` was created from the CI-green commit `236cf98`.
What the release proves, checked against the published artifacts rather than the build log:
- **The APK is signed by the real release key.** `apksigner verify --print-certs` on the
  downloaded `buy-my-way-v0.9.0.apk` gives `O=Horizon Studio - Lukasz Gorny`, SHA-256
  `0defec73…c7db` and SHA-1 `bbe365e6…4aaf` — the same two fingerprints that are now in
  `assetlinks.json` and in the Firebase OAuth client. All three places agree.
- **The published SHA-256 matches the published APK**, byte for byte
  (`a18558dd673ebf530dfd60bcfa6e6c5c0667057d9eaa5025d95efc0e3d933e07`). That is acceptance
  criterion 1's checksum half, on the real artifact.
- **The version comes from the tag**: `versionName 0.9.0`, `versionCode 90000` — decision 28's
  arithmetic, and evidence that the workflow's `fetch-depth: 0` does its job.
- **git-cliff wrote `CHANGELOG.md` back to `main`** (`2a1a8bc`) with a `[0.9.0]` section built
  from the commit messages, and it was merged back to `dev` and pushed.
**Still not verified, and now it is only what a phone can answer:** installing `v1.0.0` over
`v0.9.0` (needs the second tag); the update banner offering a *real* release and „Pobierz"
installing it; Google sign-in and App Link verification under the **release** signature, both
of which a debug build cannot stand in for.
- ~~**A real tag.**~~ ~~**The `.sha256` a person compares.**~~ Both done by `v0.9.0`, above.
- **Acceptance criterion 1, second half** („installing over the previous release succeeds"):
  `v0.9.0` is now the previous release to install `v1.0.0` over, so this is testable at last,
  but it has not been tested — it needs the second tag.
- **Acceptance criterion 2, the update banner on a real older build**: half of it is now
  possible for the first time. A phone carrying `v0.9.0` should be offered `v1.0.0` the moment
  that Release exists. On the emulator the banner was driven against a handed-over document and
  the live HTTP call was seen answering `404`; what is untested is „Pobierz" actually
  downloading a real APK and the installer taking it.
- **Google sign-in and the App Link under the release signature.** Both were checked on debug
  builds, which cannot stand in for them: sign-in matches the certificate of the APK, and a
  phone verifies an App Link against the key that signed the build it has. `v0.9.0` on a phone
  answers both, and nothing before it could.
- The Linux machine (open question 2).
